...
def process():
    ...
    input_dir = filedialpy.openDir(
        title="Choose Input Folder", initial_dir=get_last_dir()
    )
    ...
    for tiff_file in tiff_files:
        ...
        image_bin = cv2.threshold(
            image_gray, 0, 255, cv2.THRESH_OTSU + cv2.THRESH_BINARY
        )[1]
        ...
        dists = scipy.spatial.distance.squareform(
            scipy.spatial.distance.pdist(mask_points)
        )
        ...
        mapped_corners = np.array(
            [inv_matrix @ (corner - offset) for corner in corners]
        )
        ...
        output_array[
            pad_top : pad_top + crop_height, pad_left : pad_left + crop_width
        ] = cropped_array
        ...
        if dev_mode:
            ...
            axs[0].add_patch(
                Circle(
                    (center_of_mass[1], center_of_mass[0]), 5, color="white", fill=True
                )
            )
            axs[0].add_patch(
    ...
    if len(cropped_images) > 0:
        ...
        montage_array = np.zeros(
            (
                parameters["export_height"] * n_rows,
                parameters["export_width"] * n_cols,
                3,
            ),
            dtype=np.uint8,
        )
        ...
        for i, (tiff_path, cropped_image) in enumerate(cropped_images):
            ...
            col = i % n_cols
            montage_array[
                row * parameters["export_height"] : (row + 1)
                * parameters["export_height"],
                col * parameters["export_width"] : (col + 1)
                * parameters["export_width"],
            ] = cropped_image
    ...
    shutil.copy(parameter_path, os.path.join(input_dir, "parameter.toml"))

process()
while True:
    user_input = input("Press 'Enter' to continue. Enter 'q' to quit.")
